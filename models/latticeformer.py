...
class Latticeformer(torch.nn.Module):
    ...
    # def forward(self, data):  ## 为了cross-attn修改
    def forward(self, data, return_tokens=False):
        """
        Args:
            data: PyG Data object batch
            return_tokens (bool): whether to return [N, D] token-level output and batch index
        Returns:
            If return_tokens: Tuple (x: [N, D], batch: [N])
            Else: Tensor [B, D]
        """
        x = data.x
        pos = data.pos
        ...
        # print(f"x before: {x}")  ### debug: None
        x = self.input_embeddings(x)
        # print(f"x after: {x}")  ### debug

...
class MultimodalDatasetMP_Latticeformer(MultimodalDatasetMP):  # 这个是原有的可以加入.xrd衍射图的模块
...
###############################
# Model Fusion with Text
###############################

def make_multimodal_data(material, ATOM_NUM_UPPER, cell_format:CellFormat):
    if "final_structure" in material:
        structure = material['final_structure']
    elif "structure" in material:
        structure = material['structure']
    ...
    atom_pos = torch.tensor(structure.cart_coords, dtype=torch.float)
    atom_fea = generate_site_species_vector(structure, ATOM_NUM_UPPER)
    data = Data(x=atom_fea, y=None, pos=atom_pos)  # 改data的目标结构 -> add text
    # data = Data(x=.., y=.., pos=.., trans_vec=.., text=.., ..)
    # or: data.text = ...
    data.trans_vec = torch.tensor(structure.lattice.matrix, dtype=torch.float)[None]
    data.material_id = id
    data.sizes = torch.tensor([atom_pos.shape[0]], dtype=torch.long)
...
### add new dataset processing class for structure+text
class FusionDatasetMP_Latticeformer(MultimodalDatasetMP):
    def __init__(self, params, target_split, target_set=None, post_filter=None):
        self.use_primitive = params.use_primitive if hasattr(params, 'use_primitive') else True
    ...
    def process_input(self, material):
        return make_data(material, self.ATOM_NUM_UPPER, self.use_primitive)  # add text
        
        ##### get & add text
        ##### 如果需要做数据增强就再从对应的一组description里面shuffle
        # description = material...  
        # return make_multimodal_data(..., text = description)
